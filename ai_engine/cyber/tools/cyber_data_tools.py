...
def _load_data():
    # Load the cyber incidents dataset from SQLite database
    try:
        engine = create_engine(DB_URL)
        df = pd.read_sql("SELECT * FROM cyber_events", engine)
        # Ensure lowercase column names for consistency
        df.columns = df.columns.str.lower().str.strip()
        return df
    except Exception as e:
        return f"Error loading data from database: {str(e)}"
